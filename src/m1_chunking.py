...
def chunk_hierarchical(text: str, parent_size: int = HIERARCHICAL_PARENT_SIZE,
    ...
    def add_parent_and_children(p_text: str, idx: int):
        p_text = p_text.strip()
        if not p_text:
            return
        pid = f"parent_{idx}"
        ...
        # Split parent into child chunks <= child_size chars
        p_paras = [p.strip() for p in p_text.split("\n\n") if p.strip()]
        for para in p_paras:
            if len(para) <= child_size:
                children.append(Chunk(text=para, metadata={**metadata, "chunk_type": "child"}, parent_id=pid))
            else:
                sents = [s.strip() for s in re.split(r'(?<=[.!?])\s+', para) if s.strip()]
                curr_child = ""
                for s in sents:
                    if len(curr_child) + len(s) + 1 > child_size and curr_child:
    ...
    for para in paragraphs:
        if len(current_parent_text) + len(para) + 2 > parent_size and current_parent_text:
            add_parent_and_children(current_parent_text, len(parents))
...
def chunk_structure_aware(text: str, metadata: dict | None = None) -> list[Chunk]:
    ...
    for part in parts:
        ...
        if re.match(r'^#{1,3}\s+', part_strip):
            ...
            current_content = f"{current_content}\n\n{part_strip}".strip() if current_content else part_strip
