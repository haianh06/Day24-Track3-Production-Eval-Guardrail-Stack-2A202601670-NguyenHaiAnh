...
import os, sys, json as _json, re
    ...
    if client:
        try:
            resp = client.chat.completions.create(
                model=LLM_MODEL,
                messages=[
                    {"role": "system", "content": "Tóm tắt đoạn văn sau trong 2-3 câu ngắn gọn bằng tiếng Việt. Chỉ trả về nội dung tóm tắt, không giải thích thêm."},
    ...
    if client:
        try:
            resp = client.chat.completions.create(
                model=LLM_MODEL,
                messages=[
                    {"role": "system", "content": 'Trích xuất metadata từ đoạn văn. Chỉ trả về JSON hợp lệ, không có markdown: {"topic": "...", "entities": ["..."], "category": "policy|hr|it|finance", "language": "vi|en"}'},
                    {"role": "user", "content": text},
                ],
                max_tokens=150,
            )
...
def _enrich_single_call(text: str, source: str) -> dict:
    ...
    if client:
        try:
            prompt = '''Phân tích đoạn văn và trả về JSON hợp lệ (không có markdown):
{
